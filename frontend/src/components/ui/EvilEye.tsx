...
export function EvilEye({
  ...
  // Store latest prop values in ref so WebGL never tears down on prop changes
  ...
  useEffect(() => {
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPupilSize: { value: propsRef.current.pupilSize },
        uIrisWidth: { value: propsRef.current.irisWidth },
        ...
        uMouse: { value: [0, 0] },
        uPupilFollow: { value: propsRef.current.pupilFollow },
        uFlameSpeed: { value: propsRef.current.flameSpeed },
        uEyeColor: { value: hexToVec3(propsRef.current.eyeColor) },
        uBgColor: { value: hexToVec3(propsRef.current.backgroundColor) },
      },
    });
    ...
    function update(time: number) {
      ...
      // Organic Lerp for intensity, glow, scale, and flame speed (60 FPS smooth transitions)
      const targetProps = propsRef.current;
      
      const curIntensity = program.uniforms.uIntensity.value;
      program.uniforms.uIntensity.value += (targetProps.intensity - curIntensity) * 0.04;

      const curGlow = program.uniforms.uGlowIntensity.value;
      program.uniforms.uGlowIntensity.value += (targetProps.glowIntensity - curGlow) * 0.04;

      const curScale = program.uniforms.uScale.value;
      program.uniforms.uScale.value += (targetProps.scale - curScale) * 0.04;

      const curFlame = program.uniforms.uFlameSpeed.value;
      program.uniforms.uFlameSpeed.value += (targetProps.flameSpeed - curFlame) * 0.04;

      program.uniforms.uEyeColor.value = hexToVec3(targetProps.eyeColor);
  ...
  }, []); // Run ONCE on mount, NEVER teardown WebGL on prop updates!
